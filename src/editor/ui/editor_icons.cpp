...
// EditorIcon -> Lucide codepoint (lucide-static font/info.json). 0 = no
// mapping, fall back to the vector glyph.
...
void drawEditorIcon(ImDrawList* dl, EditorIcon icon, ImVec2 c, float r, ImU32 col) {
    ...
    if (s_iconFont) {
        if (const ImWchar cp = iconCodepoint(icon)) {
            ...
            // matches the old vector glyphs' visual extent.
    ...
    switch (icon) {
        ...
        case EditorIcon::UICanvas:
        case EditorIcon::UIText:
        case EditorIcon::UIImage:
        case EditorIcon::UIButton:
        case EditorIcon::LightRect:
        case EditorIcon::LightDisk:
        case EditorIcon::Cube:
        case EditorIcon::Sphere:
        case EditorIcon::Plane:
        case EditorIcon::Pyramid:
        case EditorIcon::Cone:
        case EditorIcon::Triangle:
        case EditorIcon::Empty:
        ...
        case EditorIcon::Probe: {
            // Reflective sphere: a circle with a specular glint dot up-left.
            dl->AddCircle(c, 0.66f * r, col, 0, th);
            dl->AddCircleFilled(P(-0.24f, -0.24f), 0.16f * r, col);
            break;
        }
        case EditorIcon::Volume: {
            // Probe grid in a box: the irradiance volume's sample lattice.
            dl->AddRect(P(-0.70f, -0.62f), P(0.70f, 0.62f), col, 0.0f, 0, th);
            for (int gy = -1; gy <= 1; gy += 2)
            for (int gx = -1; gx <= 1; ++gx)
                dl->AddCircleFilled(P(0.36f * gx, 0.26f * gy), 0.10f * r, col);
            break;
        }
        case EditorIcon::Decal: {
            // A projected splat: box outline with a filled droplet inside.
            dl->AddRect(P(-0.68f, -0.68f), P(0.68f, 0.68f), col, 0.0f, 0, th);
            dl->AddCircleFilled(P(0.08f, 0.10f), 0.30f * r, col);
            dl->AddCircleFilled(P(-0.28f, -0.26f), 0.14f * r, col);
            break;
        }
        case EditorIcon::Particle: {
            // Rising spray: three dots shrinking with height.
            dl->AddCircleFilled(P(-0.10f, 0.44f), 0.26f * r, col);
            dl->AddCircleFilled(P(0.34f, -0.02f), 0.18f * r, col);
            dl->AddCircleFilled(P(-0.26f, -0.44f), 0.12f * r, col);
            break;
        }
        case EditorIcon::UIWidget: {
            // A button: rounded rect with a text rule inside.
            dl->AddRect(P(-0.72f, -0.44f), P(0.72f, 0.44f), col, 0.22f * r, 0, th);
            dl->AddLine(P(-0.38f, 0.0f), P(0.38f, 0.0f), col, th);
            break;
        }
