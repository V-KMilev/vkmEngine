...
#include "texture/gl_texture.h"  // qualified: a resource/gl_texture.h (Engine GLTexture) shadows the bare name
 ...
 * is RAII now, so there is no manual GL cleanup.
 *
 * The bake pass calls createTargets() once, then drives the bake purely
 * through the bindX() / attachXFace() / generateEnvMips() ops below - it
 * never touches raw GL. The forward + skybox passes sample via bindEnvCube /
 * bindIrradiance / bindPrefilter / bindBrdf. (Re)bake is gated on the
 * source path.
 */
class GLIBL {
    public:
