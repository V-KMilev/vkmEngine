 ...
 *
 * Runs after the forward draw, into the HDR target, at the far plane (depth
 * func LEQUAL, no depth write), so it fills only background pixels (depth ==
 * far from the prepass) without disturbing lit geometry. Outputs linear
 * radiance - the composite pass tone-maps it together with lit geometry.
 * No-ops until the IBL bake is ready.
 */
