 ...
 * shape GLBloom needs (Core::Texture2D doesn't model a per-level render-target
 * chain, so the raw GL is kept encapsulated here). Subclasses own the format /
 * filter / sizing policy via their own resize() + createChain(); this base owns
 * the handles, their release, and the bind / attach / query ops.
 *
 * Not used polymorphically (callers hold a concrete GLBloom), so the dtor is
 * intentionally non-virtual - it still runs on concrete destruction.
 */
