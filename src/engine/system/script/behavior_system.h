...
class BehaviorSystem : public System {
    ...
    public:
         ...
         *
         * Used to tear down a whole scene's running behaviors: on play stop
         * (before the snapshot swaps the played scene away) and at shutdown.
         * Static because the editor's stop path has no BehaviorSystem handle -
         * friendship with Behavior is class-wide, so a static reaches its flags.
         */
    ...
    private:
        /// Fire onStart once (binding context first), catching + disabling on throw.
        static void ensureStarted(Behavior& behavior, EntityId entity, Scene& scene, ResourceManager& resources);
