...
uint32_t BVH::buildRecursive(uint32_t start, uint32_t end) {
    ...
    float leafCost = static_cast<float>(primCount);
    ...
    uint32_t leftChild = buildRecursive(start, midIdx);
...
float BVH::findBestSplit(uint32_t start, uint32_t end, int& axis, float& splitPos) const {
    ...
    // Try each axis
    for (int a = 0; a < 3; ++a) {
        float axisMin = centroidBounds.min[a];
        float axisMax = centroidBounds.max[a];
        ...
        // Evaluate splits
        float parentArea = computeBounds(start, end).surfaceArea();
        ...
        for (int i = 0; i < SAH_BUCKETS - 1; ++i) {
            ...
            float leftArea = boundsLeft.surfaceArea();
            float rightArea = boundsRight[i + 1].surfaceArea();
...
#ifdef __SSE2__
bool BVH::isNodeVisibleSIMD(const BVHNode& node, const Frustum& frustum) const {
    // Load node bounds into SIMD registers
    __m128 nodeMinX = _mm_set1_ps(node.minX);
    __m128 nodeMinY = _mm_set1_ps(node.minY);
    __m128 nodeMinZ = _mm_set1_ps(node.minZ);
    __m128 nodeMaxX = _mm_set1_ps(node.maxX);
    __m128 nodeMaxY = _mm_set1_ps(node.maxY);
    __m128 nodeMaxZ = _mm_set1_ps(node.maxZ);

    // Test first 4 planes at once
    __m128 planeNx = _mm_set_ps(frustum.planes[3].x, frustum.planes[2].x, frustum.planes[1].x, frustum.planes[0].x);
    __m128 planeNy = _mm_set_ps(frustum.planes[3].y, frustum.planes[2].y, frustum.planes[1].y, frustum.planes[0].y);
    __m128 planeNz = _mm_set_ps(frustum.planes[3].z, frustum.planes[2].z, frustum.planes[1].z, frustum.planes[0].z);
    __m128 planeD  = _mm_set_ps(frustum.planes[3].w, frustum.planes[2].w, frustum.planes[1].w, frustum.planes[0].w);

    // Select p-vertex components based on plane normal signs
    __m128 zero = _mm_setzero_ps();
    __m128 pX = _mm_or_ps(_mm_and_ps(_mm_cmpge_ps(planeNx, zero), nodeMaxX),
                          _mm_andnot_ps(_mm_cmpge_ps(planeNx, zero), nodeMinX));
    __m128 pY = _mm_or_ps(_mm_and_ps(_mm_cmpge_ps(planeNy, zero), nodeMaxY),
                          _mm_andnot_ps(_mm_cmpge_ps(planeNy, zero), nodeMinY));
    __m128 pZ = _mm_or_ps(_mm_and_ps(_mm_cmpge_ps(planeNz, zero), nodeMaxZ),
                          _mm_andnot_ps(_mm_cmpge_ps(planeNz, zero), nodeMinZ));

    // Compute dot product + d for all 4 planes
    __m128 dot = _mm_add_ps(_mm_add_ps(_mm_mul_ps(planeNx, pX), _mm_mul_ps(planeNy, pY)),
                            _mm_add_ps(_mm_mul_ps(planeNz, pZ), planeD));

    // If any result < 0, AABB is outside that plane
    int mask = _mm_movemask_ps(_mm_cmplt_ps(dot, zero));
    if (mask != 0) return false;

    // Test remaining 2 planes (near/far) with scalar code
    for (int i = 4; i < 6; ++i) {
        const auto& plane = frustum.planes[i];
        float px = (plane.x >= 0.0f) ? node.maxX : node.minX;
        float py = (plane.y >= 0.0f) ? node.maxY : node.minY;
        float pz = (plane.z >= 0.0f) ? node.maxZ : node.minZ;
...
#ifdef __SSE2__
    return isNodeVisibleSIMD(node, frustum);
#else
...
std::vector<uint32_t> BVH::queryFrustum(const Frustum& frustum) const {
...
std::vector<BVH::NodeBounds> BVH::getNodeBounds(int maxDepth, bool leavesOnly) const {
