...
void Engine::run() {
    ...
    while (m_window.beginFrame()) {
        ...
        // First frame after init: optionally migrate the backend context to
        // a dedicated render thread + locate the RenderSystem so the
        // overlap loop can split its buildView / executeFrame phases. Done
        // here rather than before the loop so initSystems (and any backend
        // work it triggers, like the IBL bake's first execute) still sees
        // the context on the main thread.
        if (m_renderThreadEnabled && !m_renderThread) {
            ...
            if (!m_renderSystem) {
                ...
                // (currently just EditorSystem). Their executeBackend()
                // runs on the render thread after the scene render
                // finishes, so the UI overlays the rendered scene.
                ...
                // Editor panels can no longer run backend work inline (the
                // context moves off main); RenderSystem queues requests
                // instead and the render thread drains them in executeFrame.
                m_renderSystem->setBackendOnSeparateThread(true);
        ...
        if (m_renderThread) {
            ...
            // do their build on main here; their executeBackend() is
            // appended to the render-thread lambda below.
            ...
            //    affecting the render thread's snapshot. UI-stage backend-
            //    work systems run AFTER executeFrame so the UI overlays
            //    the rendered scene.
...
void Engine::updateStage(SystemStage stage, FrameContext& ctx) {
    ...
    for (auto& system : m_systemsByStage[static_cast<size_t>(stage)]) {
        if (!system->isEnabled()) continue;
        system->update(ctx);
        // Single-threaded path: backend companion runs inline here.
        // Skipped when render thread is enabled - the run loop routes
        // these into the render-thread lambda instead.
        if (!m_renderThread && system->hasBackendWork()) {
            system->executeBackend(ctx);
        }
    }
}
